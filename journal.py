# Copied from https://github.com/ejrh/backup
# I thank Edmund Horner for writing a nice USN library to read the journal
...
def normalise(path):
    ...
    # Maintain case even on Windows because we do case sensitive matching in other locations
    # Keep path separators as they are, do not convert
    double_sep = os.path.sep + os.path.sep
    path = path.replace(double_sep, os.path.sep)
...
def get_ancestors(path):
    ...
    ancestors = []
    components = normalise(path).split(os.path.sep)
    subpath = os.path.sep
    for c in components:
        if subpath == '':
            subpath = c
...
class FrnMap(object):
    ...
    def build_path(self, frn):
        ...
        return os.path.join(self.build_path(parent_frn), name)
...
class Journal(object):
    ...
    def process_usn(self, tup, fn):
        ...
        try:
            path = os.path.join(parent_path, fn)
    ...
    def process(self, notifier=default_notifier):
        ...
        if self.journal_id != queried_journal_id or first_usn > self.last_usn:
            if self.journal_id is None:
                notifier('Journal is new (available id 0x%016x, first available USN 0x%016x)' % (
                queried_journal_id, first_usn))
            else:
                notifier(
                    'Journal is too new (recorded id 0x%016x, available id 0x%016x, last recorded USN 0x%016x, first available USN 0x%016x)' % (
                    self.journal_id, queried_journal_id, self.last_usn, first_usn))
        ...
        for tup, fn in generate_journal(volh, self.journal_id, start_usn):
            if self.replay_all or self.last_usn < tup[5]:
                self.process_usn(tup, fn)
                self.last_usn = tup[5]
            pct = int(100 * (tup[5] - start_usn) / (next_usn - start_usn))
            if pct > last_pct:
